...
func (tm *TriggerMatcher) Match(keyPath string) bool {
    ...
    // Match returns true if keyPath matches On condition AND does not contain underscore
    // Quanatee uses _ to add a prefix with _ as separator to denote raw, unprocessed data
    if matched && strings.Contains(keyPath, "_") == false {
        return true
    } else {
        return false
    }

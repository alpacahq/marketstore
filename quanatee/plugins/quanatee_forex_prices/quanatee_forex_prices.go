...
func GetIntrinioPrices(symbol string, from, to time.Time, realTime bool, period string, token string) (Quote, error) {
    ...
    for i, j := 0, len(quote.Epoch)-1; i < j; i, j = i+1, j-1 {
        quote.Epoch[i], quote.Epoch[j+1] = quote.Epoch[j+1], quote.Epoch[i]
        quote.Open[i], quote.Open[j+1] = quote.Open[j+1], quote.Open[i]
        quote.High[i], quote.High[j+1] = quote.High[j+1], quote.High[i]
        quote.Low[i], quote.Low[j+1] = quote.Low[j+1], quote.Low[i]
        quote.Close[i], quote.Close[j+1] = quote.Close[j+1], quote.Close[i]
    }

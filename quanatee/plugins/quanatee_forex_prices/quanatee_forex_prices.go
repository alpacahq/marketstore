        ...
        if !firstLoop {
            ...
            */
            ...
            for _, symbol := range symbols {
                ...
                log.Info("Forex: %v row(s) to %s/%s/OHLC from %v to %v", len(quote.Epoch), quote.Symbol, tiifx.baseTimeframe.String, time.Unix(quote.Epoch[0], 0), time.Unix(quote.Epoch[len(revQuote.Epoch)-1], 0))
            ...
            for _, quote := range quotes {
                ...
                if revSymbol != "" {
                    ...
                    log.Info("Forex: %v row(s) to %s/%s/OHLC from %v to %v", len(revQuote.Epoch), revQuote.Symbol, tiifx.baseTimeframe.String, time.Unix(revQuote.Epoch[0], 0), time.Unix(revQuote.Epoch[len(revQuote.Epoch)-1], 0))

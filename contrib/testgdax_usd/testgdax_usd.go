...
import (
	...
	testgdax "github.com/creslinux/go-gdax"
)

type ByTime []testgdax.HistoricRate
...
func (gd *GdaxFetcher) Run() {
	...
	for _, symbol := range symbols {
		tbk := io.NewTimeBucketKey("testgdax_usd_" + symbol + "/" + gd.baseTimeframe.String + "/OHLCV")
	...
	for {
		...
		lastTime := timeStart
		for _, symbol := range symbols {
			params := testgdax.GetHistoricRatesParams{
				Start:       timeStart,
				End:         timeEnd,

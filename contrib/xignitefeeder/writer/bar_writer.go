...
type BarWriter interface {
	Write(resp api.GetBarsResponse) error
}
...
func (b BarWriterImpl) Write(bars api.GetBarsResponse) error {
	// convert Bar Data to CSM (ColumnSeriesMap)
	csm, err := b.convertToCSM(bars)
	if err != nil {
...
func (b *BarWriterImpl) convertToCSM(resp api.GetBarsResponse) (io.ColumnSeriesMap, error) {
	...
	for _, bar := range resp.ArrayOfBar {
		...
		// When Volume is 0, xignite getBarsRange API may return data with open:0, close:0, high:0, low:0.
	...
	tbk := io.NewTimeBucketKey(resp.Security.Symbol + "/" + b.Timeframe + "/OHLCV")
